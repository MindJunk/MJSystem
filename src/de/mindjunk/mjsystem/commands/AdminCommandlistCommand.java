		...
		if(sender instanceof Player) {
			...
			if(moduleenabled == true) {
				if(args.length == 0) {
					...
					if(player.hasPermission("mjs.acmdlist.set")) {
						...
						plugin.saveConfig();
						player.sendMessage(prefix + "§7Du hast den AdminCommandlist-Link zu §6" + args[0] + " §7geändert!");
						System.out.println(ChatColor.WHITE + "[MJSystem]>> Der Spieler " + ChatColor.BLUE + player.getName() + ChatColor.WHITE + " hat den AdminCommandlist-Link zu " + ChatColor.BLUE + args[0] + ChatColor.WHITE + " geändert");
			...
			if(moduleenabled == true) {
				if(args.length == 0) {
					...
					plugin.saveConfig();
					sender.sendMessage(prefix + "§7Du hast den AdminCommandlist-Link zu §6" + args[0] + " §7geändert!");
				} else
